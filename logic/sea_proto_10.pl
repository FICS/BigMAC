...
%     v   v       |
%     n4->n5<------
% Prolog Documention: http://rigaux.org/language-study/syntax-across-languages-per-language/Prolog.html
% https://www.swi-prolog.org/pldoc/man?section=funcsummary

% sub(node, uid, gid, user, group, other, cap)
...
% obj(node, uid, gid, user, group, other, ext)
% EXT for object is a list of external attack surfaces
% A complete ext list might be [usb,bt,nfc,bb,wifi]
% usb: 1
% bt: 2
% nfc: 3
% wifi: 4
% bb: 5
obj(n2, 1, 1, 7, 7, 7, [1,2]).
obj(n5, 1, 1, 7, 7, 7, [1,3]).
obj(n6, 1, 1, 7, 7, 7, [1,4,5]).
obj(n8, 2, 2, 4, 4, 4, [1,2]).	% Others can read
...
% Self edges
% NOTE: ths would cause troubles for wildcard query
%edge(n5, n5).
%edge(n8, n8).
...
travel(A,B,Visited,Path,Cut) :-
	edge(A,C), % for any edge starting from node A
	C \== B, % continue we we havent reached the goal, B
	\+member(C,Visited), % has NOT been visited
	length(Visited,Len), % get path length
	Len < Cut,	% NOTE: cannot wildcard for cutoff in the query
	% keep going if path length is less than the cutoff
